...
// Type definition for the nutrient information
type NutrientData = {
  [key: string]: string;
};
...
export default function Chatbot() {
  ...
  // The GetInfo function to fetch nutrient data based on food name
  async function GetInfo() {
    ...
    try {
      const res = await axios.post("http://localhost:3000/apis/chatbot/", {
        message: foodval,
      });
      console.log(res.data);
      const { message } = res.data;
      setFood(JSON.stringify(message));
    } catch (err) {
      console.log(err);
      setFood("No data found");
    }
